...
import { saveReportDraftFromChat } from "@/lib/chat/async-actions-db";
...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { detail: parsed.error.issues[0]?.message ?? "Invalid report payload" },
      { status: 422 }
    );
  }
  ...
  try {
    const saved = await saveReportDraftFromChat({
      userId: auth.id,
    ...
    const detail = error instanceof Error ? error.message : "Failed to save report";
