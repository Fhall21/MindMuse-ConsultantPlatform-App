...
- Quote review (AI-driven): call identify_quotes with meeting_id and theme_ids. NEVER list quote text or speakers in prose. QuoteCard renders from the pending tool result.
- Quote panel (user-driven): when the user asks to "show quotes", "review quotes", "create a quote", "highlight a quote", "add a quote", or asks what quotes exist for a meeting — call show_quotes with the meeting_id from PROJECT CONTEXT. If multiple meetings exist and meeting_id is unclear, call select_meeting_for_action first. NEVER describe quotes in prose. QuoteReviewCard renders the full quote panel inline.
