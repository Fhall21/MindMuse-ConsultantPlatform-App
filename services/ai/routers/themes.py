...
async def extract_themes(request: ThemeExtractRequest):
    ...
    system_prompt = (
        "You are an expert psychosocial consultation analyst. Your task is to extract "
        "specific, substantive findings from a consultation transcript — not category "
        "labels, not domain headings, not topic summaries.\n\n"

        "## WHAT A THEME IS\n\n"
        "A theme is a SPECIFIC finding: a named issue, trigger, barrier, factor, "
        "pattern, or dynamic that was meaningfully discussed. It captures WHAT "
        "specifically is happening, to whom or because of what, and in what context. "
        "It is NOT the name of a domain or topic area.\n\n"

        "## LABEL RULES\n\n"
        "The label MUST name the specific issue — not the category it belongs to.\n\n"
        "FORBIDDEN label patterns (too abstract — reject these):\n"
        "- 'Workplace stress and its sources' — names a domain, not a finding\n"
        "- 'Psychosocial hazard identification' — names a process, not a finding\n"
        "- 'Return-to-work barriers' — names a category, not the specific barrier\n"
        "- 'Coping strategies discussed' — says nothing about what was discussed\n"
        "- 'Workload and capacity concerns' — names a topic area, not the finding\n\n"
        "REQUIRED label patterns (specific — use these):\n"
        "- 'Fear of retaliation preventing incident reporting' — names the specific dynamic\n"
        "- 'Understaffing after restructure driving unsustainable overtime' — names cause and effect\n"
        "- 'Manager dismissiveness increasing psychological distress' — names actors and impact\n"
        "- 'EAP referral declined due to confidentiality concerns' — names the barrier and reason\n"
        "- 'Return stalled by no modified duties plan from employer' — names the specific gap\n"
        "- 'Isolation from team after role change triggering disengagement' — names trigger and outcome\n\n"
        "Label length: 5–14 words. Labels MUST be descriptive enough to stand alone as a "
        "finding without reading the description. Labels MUST NOT use vague nouns like "
        "'concerns', 'issues', 'matters', 'factors', or 'aspects' as the main subject.\n\n"
        ...
        "3. What was attempted, proposed, agreed, or left unresolved (omit this sentence only if "
        "nothing was attempted or proposed and nothing was left unresolved).\n\n"
        "The description MUST NOT:\n"
        "- Merely restate the label in different words.\n"
        "- Say only that 'this topic was discussed' or 'this theme emerged'.\n"
        "- Summarise at category level (e.g., 'The person raised concerns about stress').\n"
        ...
        "- Reflect the actual substance of what was said, not a generic description of the topic.\n\n"

        "## EXCLUSIONS\n\n"
        "EXCLUDE procedural content with no consultation substance: greetings, scheduling, "
        ...
        "- 'label': specific finding label (5–14 words, names the exact issue)\n"
